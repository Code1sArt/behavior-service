...
export class BehaviorsService {
    ...
    async getBehaviorHistory(dateStr?: string, classroomId?: number, studentId?: string, termId?: number) {
        ...
        const whereFilter: any = {
            AND: [
                // 1. กรองตาม Term ID (ต้องเจาะผ่าน Student -> Classroom)
                termId ? {
                    student: {
                        classroom: { termId: Number(termId) }
                    }
                } : {},

                // 2. กรองตาม Classroom
                classroomId ? {
                    student: { classroomId: Number(classroomId) }
                } : {},
            ...
            try {
                ...
                await this.prisma.behaviorRecord.create({
                    data: {
                        ...
                        // เชื่อมโยงผู้บันทึก (แอดมิน/ครู)
                        recorder: {
                            connect: { id: recorderId },
                        },
                        // เชื่อมโยงหมวดหมู่คะแนน (ถ้าระบุ categoryId มา)
                        ...(row.categoryId && {
                            category: {
